...
class SimpleX(BaseModel):
    ...
    def forward(self, inputs, training=True):
        ...
        item_vecs_batch = item_vecs.view(user_vecs.size(0), self.num_negs + 1, -1) if training else item_vecs.unsqueeze(0).repeat(user_vecs.size(0),1,1)

        y_pred = torch.bmm(item_vecs_batch,user_vecs.unsqueeze(-1)).squeeze(-1)
